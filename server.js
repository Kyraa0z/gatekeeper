...
app.get('/gatekeeper/verify', (req, res) => {
...
function checkSocketRate(ip) {
    const now = Date.now(), window = 60000, max = 5;
    const entry = socketRateMap.get(ip);
    if (!entry || now > entry.resetAt) { socketRateMap.set(ip, { count: 1, resetAt: now + window }); return true; }
    if (entry.count >= max) return false;
    entry.count++;
    return true;
}
setInterval(() => { const now = Date.now(); for (const [ip, e] of socketRateMap.entries()) if (now > e.resetAt) socketRateMap.delete(ip); }, 300000);

initDB();
const bot = initBot(process.env.TELEGRAM_BOT_TOKEN, process.env.ADMIN_CHAT_ID, io);

function generateAndSaveToken(sessionId, ip) {
    const token = jwt.sign({ sessionId, ip }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRES_IN || '7d' });
...
io.on('connection', (socket) => {
    ...
    db.get("SELECT 1 FROM blocked_ips WHERE ip = ? UNION SELECT 1 FROM blocked_devices WHERE device_id = ?", [ip, deviceId], (err, bannedRow) => {
        if (bannedRow) { accessLogger.warn(`Engelli: ip=${ip}`); socket.emit('status_update', { status: 'blocked' }); return socket.disconnect(); }
        db.get("SELECT status, jwt_token FROM visitors WHERE session_id = ?", [sessionId], (err, row) => {
            if (!row) return;
            if (row.status === 'approved' && row.jwt_token) socket.emit('status_update', { status: 'approved', token: row.jwt_token });
            else socket.emit('status_update', { status: row.status });
        });
    });

    socket.on('request_access', async (data) => {
        if (!checkSocketRate(ip)) { accessLogger.warn(`Rate limit: ip=${ip}`); return socket.emit('rate_limited', { message: 'Çok fazla istek. 1 dakika bekleyin.' }); }
        db.get("SELECT 1 FROM blocked_ips WHERE ip = ? UNION SELECT 1 FROM blocked_devices WHERE device_id = ?", [ip, deviceId], async (err, bannedRow) => {
            ...
            try {
                ...
                if (!vResult.success) { accessLogger.warn(`Turnstile başarısız: ip=${ip}`); return socket.emit('status_update', { status: 'idle' }); }
                const note = data.note ? data.note.substring(0, 200) : '';
                const ua   = socket.handshake.headers['user-agent'];
                db.run("INSERT OR REPLACE INTO visitors (session_id, device_id, ip, user_agent, status) VALUES (?, ?, ?, ?, 'pending')", [sessionId, deviceId, ip, ua], (err) => {
                    if (err) { systemLogger.error(`Kayıt hatası: ${err.message}`); return socket.emit('status_update', { status: 'idle' }); }
                    const msgText = bot.buildRequestMessage(ip, note, ua, sessionId);
                    const opts = { parse_mode: 'HTML', reply_markup: { inline_keyboard: [
                        [{ text: '✅ Onayla', callback_data: `approve_${sessionId}` }, { text: '❌ Reddet', callback_data: `reject_${sessionId}` }],
            ...
            } catch (e) { systemLogger.error(`request_access hatası: ${e.message}`); socket.emit('status_update', { status: 'idle' }); }
        });
    });
});
